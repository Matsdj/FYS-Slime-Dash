...
class PCoin {
  ...
  void update() {
    ...
    if (player.Collision(x - (0.5 * globalScale), y - (0.5 * globalScale), size)) {
      reset();
      coins += coinValue;
      createParticle(player.x, player.y, 0, 10, color(YELLOW), color(YELLOW), -0.5, 0, false, 30, "+"+coinValue, 1);
    }
...
class PHealth {
  ...
  void update() {
    ...
    if (player.Collision(x - (collisionCorrector * globalScale), y - (collisionCorrector * globalScale), size)) {
      ...
      createParticle(player.x, player.y, 0, 10, color(0,255,0), color(0,255,0), -0.5, 0, false, 30, "+"+GAIN_HEALTH, 1);
    }
