...
void drawAch() {
  ...
  for (int iAch = 0; iAch < achRecordCount; iAch++) {
    ...
    if (dbAch.get(iAch).scoreType == SCORE_TYPE) {
      text("Get " + dbAch.get(iAch).requiredScore + " score: progres = " + dbAch.get(iAch).progress +"/" + dbAch.get(iAch).requiredScore +" reward: "+ dbAch.get(iAch).reward, ACH_X, ACH_Y + globalScale * iAch);
    } else if (dbAch.get(iAch).scoreType == ENEMY_TYPE) {
      text("Dash through " + dbAch.get(iAch).requiredScore + " enemies: progres = " + dbAch.get(iAch).progress +"/" + dbAch.get(iAch).requiredScore +" reward: "+ dbAch.get(iAch).reward, ACH_X, ACH_Y + globalScale * iAch);
    } else if (dbAch.get(iAch).scoreType == HEART_TYPE) {
      text("Pickup " + dbAch.get(iAch).requiredScore + " hearts: progres = " + dbAch.get(iAch).progress +"/" + dbAch.get(iAch).requiredScore +" reward: "+ dbAch.get(iAch).reward, ACH_X, ACH_Y + globalScale * iAch);
    } else if (dbAch.get(iAch).scoreType == BLOCK_TYPE) {
      text("Break " + dbAch.get(iAch).requiredScore + " blocks: progres = " + dbAch.get(iAch).progress +"/" + dbAch.get(iAch).requiredScore +" reward: "+ dbAch.get(iAch).reward, ACH_X, ACH_Y + globalScale * iAch);
    }
  }
}
